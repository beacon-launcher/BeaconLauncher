...
export function installMock(): void {
  ...
  ;(window as unknown as { beacon: unknown }).beacon = {
    ...
    setProfileAvatar: async () => ({ ok: true }),
    compatibleVersions: async () => ({ ok: true, versions: ['1.21', '1.20.4', '1.20.1'], unresolved: ['SomeLocalMod.jar'] }),
    setProfileVersion: async () => ({ ok: true, migrated: ['Sodium'], failed: [] }),
    setProfileMemory: async () => ({ ok: true }),
    repairProfile: async () => ({ ok: true, removed: ['Flashback-old.jar'] }),
